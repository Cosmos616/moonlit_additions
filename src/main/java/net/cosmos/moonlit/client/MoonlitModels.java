...
public class MoonlitModels {
    ...
    public void onModelBake(ModelBakery loader, Map<ModelResourceLocation, BakedModel> map) {
        if (!registeredModels) {
            Moonlit.LOGGER.error("Additional models failed to register! Aborting baking models to avoid early crashing.");
            return;
        }
        ...
        modelConsumers.forEach((resourceLocation, bakedModelConsumer) -> {
            ...
            Moonlit.LOGGER.error("Baking model: {}", resourceLocation);
            baked.getAndIncrement();
        });
        Moonlit.LOGGER.error("Baked {} additional model(s).", baked.get());
    }
